...
def calc_diff(previous_measure: int,
              current_measure: int,
...
for i in range(len(report)):
    if i == 0:
        ...
        calc_diff(current_measure=measurement_report[i],
                  previous_measure=measurement_report[i-1]
        )

print(f"Entries: {len(report)}, Number of increases: {TOTAL_INCREASE}, number of decreases: {TOTAL_DECREASE}, number of equals: {TOTAL_EQUAL}")
